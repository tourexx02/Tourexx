...
const TripOrganizerOnboarding = () => {
  ...
  const fetchOrganizers = async () => {
    try {
      setLoading(true);
      setError('');
      // Fetch trip organizers filtered by vendorId
      const vendorId = vendor?._id || vendor?.id;
      const response = await tripOrganizerAPI.getByVendorId(vendorId);
      const vendorOrganizers = response.tripOrganizers || response || [];
      setOrganizers(vendorOrganizers);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingOrganizer) {
        ...
        toast.success('Trip Organizer added successfully! Pending admin approval.');
      }
