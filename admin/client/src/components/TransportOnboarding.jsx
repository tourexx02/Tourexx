...
const TransportOnboarding = () => {
	...
	const fetchTransports = async () => {
		try {
			...
			setTransports(response.transports || response || []);
		} catch (error) {
	...
	const filteredTransports = transports.filter((transport) => {
		...
		const matchesApproval = filterApproval === 'all' || 
		                       (filterApproval === 'pending' && !transport.approved) ||
		                       (filterApproval === 'approved' && transport.approved);
		...
		return matchesSearch && matchesLocation && matchesApproval;
	});
								...
								<div className='flex items-start justify-between mb-4'>
									<div className='flex items-center gap-3'>
										...
										<div>
											...
											{transport.vendorId && typeof transport.vendorId === 'object' && (
												<p className="text-gray-500 text-xs">Vendor: {transport.vendorId.businessName}</p>
											)}
										</div>
									</div>
