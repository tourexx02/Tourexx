...
const CustomerQuery = () => {
  ...
  const fetchBookings = async () => {
    try {
      setLoading(true);
      // Fetch bookings filtered by vendorId
      const vendorId = vendor?._id || vendor?.id;
      const response = await bookingAPI.getByVendorId(vendorId);
      if (response.success !== false) {
        const vendorBookings = Array.isArray(response) ? response : response.data || [];
      ...
      <div className='max-w-full mx-auto'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-gray-600'>
            View bookings for {vendor?.businessName}
          </p>
        </div>
