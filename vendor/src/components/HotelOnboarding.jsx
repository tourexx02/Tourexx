...
const HotelOnboarding = () => {
  ...
  const fetchHotels = async () => {
    try {
      setLoading(true);
      setError('');
      // Fetch hotels filtered by vendorId
      const vendorId = vendor?._id || vendor?.id;
      const response = await hotelAPI.getByVendorId(vendorId);
      const vendorHotels = response.hotels || response || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingHotel) {
        ...
        toast.success('Hotel added successfully! Pending admin approval.');
      }
